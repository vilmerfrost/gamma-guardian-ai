...
export interface OARConstraint {
  name: string;
  maxDose: number;
  baseDose: number; // dose at reference (12 Gy marginal)
  unit: string;
  limit: string;
...
export const baseOARData: OARConstraint[] = [
  ...
  { name: "Hjärnstam", maxDose: 12.0, baseDose: 6.2, unit: "Gy", limit: "Max", distanceMm: 8.2 },
  { name: "N. facialis", maxDose: 8.0, baseDose: 8.1, unit: "Gy", limit: "Max", distanceMm: 2.8 },
  { name: "Optisk chiasm", maxDose: 8.0, baseDose: 1.2, unit: "Gy", limit: "Max", distanceMm: 18.5 },
  { name: "Optisk nerv", maxDose: 8.0, baseDose: 2.4, unit: "Gy", limit: "Max", distanceMm: 12.0 },
  { name: "Linsen", maxDose: 5.0, baseDose: 0.8, unit: "Gy", limit: "Max", distanceMm: 25.0 },
];

const REF_DOSE = 12; // reference marginal dose
const REF_COLLIMATOR = 8; // reference collimator mm

/** Scale OAR dose based on marginal dose and collimator size */
export function calculateOARDose(
  oar: OARConstraint,
  marginalDose: number,
  collimatorMm: number
): number {
  // Dose scales linearly with marginal dose
  const doseRatio = marginalDose / REF_DOSE;
  // Larger collimator → more scatter → higher OAR dose (simplified)
...
/** Calculate conformity index (closer to 1 = better) */
...
/** Calculate healthy tissue exposure % */
...
/** Generate DVH curve points for a structure */
export function generateDVHCurve(
  ...
  for (let d = 0; d <= maxDoseAxis; d += 0.5) {
    ...
    if (type === "gtv") {
      // Steep falloff around marginal dose
...
/** Convert DVH points to SVG path */
...
/** Calculate volume from ellipse radii (mm) */
export function ellipseVolume(rx: number, ry: number, rz: number): number {
  return Math.round(((4 / 3) * Math.PI * rx * ry * rz) / 1000 * 100) / 100; // cm³
}
